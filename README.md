...
6.  **Документация API:** Полная интерактивная документация API доступна через Swagger UI. Для аутентифицированных запросов используйте кнопку "Authorize" для ввода JWT токена.
...
Для запуска всех тестов используйте Maven(если его нет то через ./mvnw):
